...
for sample in samples:
    ...
    handle.data.add_mass_lines([16, 18, 101], labels=False, color='lightgray', linewidth=1.2)
    ...
    for index, data in enumerate(handle):
        ...
        plt.plot(
            ...
            (
                ...
                + get_common_y(
                    ...
                    ref1['xy'][:, 0],
                    ref1['peak'],
                    )[mask]*handle.meta('c_16')/y0
                + get_common_y(
                    handle.data.x,
                    ref2['xy'][:, 0],
                    ref2['peak'],
                    )[mask] * handle.meta('c_18')/y0
        ...
        # Annotate graphs
        val = handle.background('y')[np.isfinite(handle.background('y'))]/y0 + i*offset
        ...
        ax.text(
            xpos,
            min(i*offset + handle.data.y[get_range(handle.data.x, xpos, 800)]/y0) - 1,
            label,
            horizontalalignment='left',
